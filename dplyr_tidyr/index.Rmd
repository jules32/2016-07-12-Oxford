...
Our `gapminder` dataset is pretty tidy.  
...
Today we're going to work with two packages by Hadley Wickham called `dplyr` and `tidyr` and how it will help you with simple data exploration, and how you can use it in combination with the `%>%` operator for more complex wrangling (including a lot of the things you would use `for` loops for). Great resources include RStudio's [data wrangling cheatsheet](http://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) and [data wrangling webinar](https://www.rstudio.com/resources/webinars/data-wrangling-with-r-and-rstudio/). 
...
> Exercise: mutate the gdp column in our Cambodia example above.
...
Let's have a look again at the structure of the gapminder data we've been working with. 
```{r, eval=FALSE}
head(gapminder)
str(gapminder)
```
...
But for practice, now let's work with these data in a wider format, maybe the way you or your colleague entered it in a spreadsheet. We'll work with it to get it back to the way we like it.
```{r}
...
tail(gap_long)
```
note: you could also write this without the %>%: 
```{r, eval=FALSE}
gap_long <- gather(data  = gap_wide, 
                   key   = obstype_year,
                   value = obs_values)
```
So this has reshaped our dataframe, but really not how we wanted it to. Very important to check. 

Notice that it didn't know that we wanted to keep `continent` and `country` untouched; we need to give it more information about which columns we want reshaped. We can do this in several ways.
